    ...
    : ISemaphore
{
    ...
    private async Task<bool> WaitAsync(org.apache.zookeeper.ZooKeeper zK, Watcher watcher, State state)
    {
        var ephemeralNodeIndex =
            Array.FindIndex(state.SortedChildren, tuple => tuple.Path == state.EphemeralNodePath);

        if (ephemeralNodeIndex == maxCount)
        {
            var childNames =
                new HashSet<string>((await zK.getChildrenAsync(semaphorePath, watcher).ConfigureAwait(false)).Children);
